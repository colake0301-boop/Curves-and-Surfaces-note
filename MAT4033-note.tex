...
\begin{exercise}
    Check the equality $\int_D \frac{\langle \nabla f, \nabla h \rangle}{\sqrt{1 + |\nabla f|^2}} du dv = \int_D -h \text{ div} \left(\frac{\nabla f}{\sqrt{1 + |\nabla f|^2}}\right) du dv$
\end{exercise}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{Scherk's First Surface.jpg}
    \caption{Scherk's First Surface}
\end{figure}
...
\begin{proof}
    \begin{enumerate}
        ...
        Recall the definition of the complex vector $\phi$:
\[
\phi = \frac{\partial X}{\partial z} = \frac{1}{2} (X_u - i X_v).
\]
We compute the sum of the squared moduli of its components:
\[
\sum_{j=1}^3 |\phi^j|^2 = \langle \phi, \bar{\phi} \rangle = \left\langle \frac{1}{2}(X_u - i X_v), \frac{1}{2}(X_u + i X_v) \right\rangle.
\]
Expanding the inner product using linearity (and noting that $X_u, X_v$ are real vectors):
\begin{align*}
\sum_{j=1}^3 |\phi^j|^2 &= \frac{1}{4} \left( \langle X_u, X_u \rangle + i\langle X_u, X_v \rangle - i\langle X_v, X_u \rangle + \langle X_v, X_v \rangle \right) \\
&= \frac{1}{4} \left( |X_u|^2 + |X_v|^2 \right) \\
&= \frac{1}{4} (E + G).
\end{align*}
Since $X$ is isothermal, we have $E = G$ and $F = 0$. The equation above simplifies to:
\[
\sum_{j=1}^3 |\phi^j|^2 = \frac{1}{4} (2E) = \frac{1}{2} E.
\]
A surface is \textbf{regular} if and only if $X_u \times X_v \neq 0$, which is equivalent to $EG - F^2 > 0$. Under the isothermal assumption ($E=G, F=0$), the regularity condition becomes:
\[
EG - F^2 = E^2 > 0 \iff E \neq 0.
\]
Combining these results:
\[
\sum_{j=1}^3 |\phi^j|^2 \neq 0 \iff \frac{1}{2}E \neq 0 \iff E \neq 0 \iff S \text{ is regular}.
\]
    \end{enumerate}
\end{proof}

\begin{exercise}
Show the third proposition above. (The solution has been added in the typed notes, but you can try this anyway)
\end{exercise}
...
\end{theorem}
\begin{proof}
To prove that $S$ is a minimal surface ($H=0$) parameterized by isothermal coordinates, we must verify two properties established in the previous propositions:
\begin{enumerate}
    \item The coordinate functions $X^j$ are \textbf{harmonic} ($\Delta X^j = 0$).
    \item The parametrization is \textbf{isothermal} ($\langle X_u, X_u \rangle = \langle X_v, X_v \rangle$ and $\langle X_u, X_v \rangle = 0$).
\end{enumerate}

\noindent \textbf{Step 1: Verify Harmonicity of $X^j$} \\
We apply \textbf{Proposition 4.14 (1)}, which states: \textit{``The Wirtinger derivatives $\frac{\partial X^j}{\partial z}$ are holomorphic $\iff$ $X^j$ are harmonic.''}

First, we compute the Wirtinger derivative of the coordinate functions. Let $\Psi^j(z) = \int_{z_0}^z \phi^j(\zeta) d\zeta$. Then $X^j = \Re(\Psi^j) = \frac{1}{2}(\Psi^j + \overline{\Psi^j})$.
Differentiating with respect to $z$:
\[
\frac{\partial X^j}{\partial z} = \frac{\partial}{\partial z} \left[ \frac{1}{2} \left( \Psi^j(z) + \overline{\Psi^j(z)} \right) \right] = \frac{1}{2} \left( (\Psi^j)'(z) + 0 \right) = \frac{1}{2} \phi^j(z).
\]
\textit{Note:} Here we see a factor of $1/2$ difference between the geometric derivative $\frac{\partial X^j}{\partial z}$ and the integrand $\phi^j$.

By construction, the functions $\phi^j(z)$ are holomorphic. Therefore, the derivatives $\frac{\partial X^j}{\partial z}$ are also holomorphic. By Proposition 4.14 (1), this implies that $\Delta X^j = 0$ (harmonic).

\vspace{1em}
\noindent \textbf{Step 2: Verify Isothermal Condition} \\
We apply \textbf{Proposition 4.14 (2)}, which states: \textit{``$X$ is isothermal $\iff$ the complex scalar $\Phi^2 := \sum (\frac{\partial X^j}{\partial z})^2$ vanishes.''}

Using the result from Step 1 ($\frac{\partial X^j}{\partial z} = \frac{1}{2}\phi^j$), we check this condition:
\[
\Phi^2 = \sum_{j=1}^3 \left( \frac{\partial X^j}{\partial z} \right)^2 = \sum_{j=1}^3 \left( \frac{1}{2}\phi^j(z) \right)^2 = \frac{1}{4} \sum_{j=1}^3 (\phi^j(z))^2.
\]
Thus, the condition simplifies to verifying that $\sum (\phi^j)^2 = 0$. We compute this explicitly using the definitions of $\phi^j$:
\begin{align*}
(\phi^1)^2 + (\phi^2)^2 + (\phi^3)^2 &= \left[ \frac{1}{2}f(1-g^2) \right]^2 + \left[ \frac{i}{2}f(1+g^2) \right]^2 + [fg]^2 \\
&= \frac{f^2}{4}(1 - 2g^2 + g^4) + \frac{i^2 f^2}{4}(1 + 2g^2 + g^4) + f^2 g^2 \\
&= \frac{f^2}{4}(1 - 2g^2 + g^4) - \frac{f^2}{4}(1 + 2g^2 + g^4) + f^2 g^2 \\
&= \frac{f^2}{4} \Big( (1 - 2g^2 + g^4) - (1 + 2g^2 + g^4) \Big) + f^2 g^2 \\
&= \frac{f^2}{4} (-4g^2) + f^2 g^2 \\
&= -f^2 g^2 + f^2 g^2 = 0.
\end{align*}
Since the sum is zero, $\Phi^2 = 0$. By Proposition 4.14 (2), the parametrization is isothermal.

\vspace{1em}
\noindent \textbf{Conclusion} \\
Since the coordinates are isothermal, the Laplacian is given by $\Delta X = 2E H \mathbf{n}$.
From Step 1, we know $\Delta X = 0$.
Assuming the surface is regular ($E \neq 0$), this forces the mean curvature $H$ to be zero.
Therefore, $S$ is a minimal surface.
\end{proof}

\begin{remark}[Alternative Approach]
One could also observe directly that $X^j$ is defined as the real part of a holomorphic function $\Psi^j(z)$. By the Cauchy-Riemann equations, the real and imaginary parts of any holomorphic function are automatically harmonic. 
\end{remark}

\begin{remark}
The Minimal Surface part of the lecture notes is not so clear and misleading. You may find that $\phi^j$ is differ from $\frac{\partial X^j}{\partial z}$ by a factor of $\frac{1}{2}$ in the Eneeper's Theorem. So we should modify the definition of $\phi^j$ in the definition above as $\phi^j=2 \frac{\partial X^j}{\partial z}$. But anyway I want to reocord the Lectures faithfully. You may modify the structure of this part by yourselves.
\end{remark}
...
Then we obtain the geodesic equations.

\vspace{1em}
\noindent \textbf{The Equations look like:}
\begin{enumerate}
    \item $\displaystyle \ddot{u} + \frac{2f'}{f}\dot{u}\dot{v} = 0$
    \item $\displaystyle \ddot{v} - \frac{ff'}{(f')^2+(g')^2}(\dot{u})^2 + \left[ \frac{f'f''+g'g''}{(f')^2+(g')^2} \right] \dot{v}^2 = 0$
\end{enumerate}

\noindent Now we study the geodesics on a surface of revolution.

\vspace{1em}
\noindent \textbf{1. General Case ($\dot{u} \neq 0$)} \\
If $\dot{u} \neq 0$ (that is, the curve is not a meridian), then:
\[ \text{``}\alpha(t) = X(u(t), v(t)) \text{ is traveling on a constant speed''} + \text{Equation 1} \implies \text{Equation 2}. \]
This means that for a general geodesic, it suffices to check Equation 1 and the constant speed condition.

\vspace{1em}
\noindent \textbf{2. The Equation 1 is equivalent to Clairaut's Relation} \\
\begin{align*}
    (\text{Eq 1}) &\iff f^2 \ddot{u} + 2ff' \dot{v} \dot{u} = 0 \\
    &\iff \frac{d}{dt}(f^2 \dot{u}) = 0 \\
    &\iff f^2 \dot{u} = \text{const}.
\end{align*}
Note that $\displaystyle \cos \theta = \frac{\langle \alpha', X_u \rangle}{|X_u|} = \frac{f^2 \dot{u}}{f} = f \dot{u}$.
Then we have:
\[ r \cos \theta = \text{const} \]
This is the \textbf{Clairaut's relationship}.

\vspace{1em}
\noindent \textbf{3. If $\gamma$ is a Meridian} \\
If $\gamma$ is a meridian and traveling at a constant speed, then it is a geodesic.
\begin{itemize}
    \item It satisfies Equation 1 (since $\dot{u}=0 \implies 0=0$).
    \item We have $\dot{v} \neq 0$ (traveling at constant speed).
\end{itemize}

\vspace{1em}
\noindent \textbf{4. If $\gamma$ is a Parallel} \\
If $\gamma$ is a parallel, then we cannot use the argument in (1).
Equation 1 holds naturally (since $\ddot{u}=0$ and $\dot{v}=0$).
But it may not be a geodesic since $\dot{v} = 0$.
Now we check Equation 2:
\[ - \frac{f(v_0)f'(v_0)}{f'(v_0)^2 + g'(v_0)^2} (\dot{u})^2 + 0 = 0 \]
Since $f(v_0) \neq 0$ and $\dot{u} \neq 0$, this implies:
\[ f'(v_0) = 0 \]
\textbf{Geometrically speaking}, it is the circle that contains the locally max/min radius.
Then the discussion of geodesics on a revolution surface is complete.
\end{example}

\begin{exercise}
If $v'\neq 0$,(that is the curve is not a parallel)
Check equation (1) plus the condition that "$\gamma$ travels at a constant speed" implies the second equation. That is, for a non-parallel curve travelling on a constant speed, we only have to check equation 1 to conclude that $\gamma$ is a geodesics.
\end{exercise}
...
\]
Where $R$ is the region bounded by the curve $\alpha$ and $\theta(s)$ is the angle between $X_u$ and $\alpha'(s)$ in $T_p S$.
\end{proposition}
...
Then we have
\[
\langle Y, Z' \rangle = \frac{1}{2\sqrt{EG}} (E_v u' - G_u v')
\]
Hence
\[
...
\begin{exercise}
    Show $\int_{\alpha} \theta'(s) ds = 2\pi$ in the proof of the proposition above. Note that here $\alpha$ is a curve on the surface, not a plane curve.
\end{exercise}
\begin{corollary}
...
\begin{exercise}
Check that for the area for the geodesic triangle in the unit sphere is sum of internal angles minus $\pi$.
\end{exercise}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{discrete Guassian.png}
    \caption{Examples of Discrete Gauss-Bonnet Theorem}
\end{figure}
...
% New content from Lec18_251126
\begin{example}
    Consider a region $R$ on the unit sphere $S^2$. Let the boundary be $\partial R = \gamma_1 \cup \gamma_2$.
    Suppose $\gamma_1$ is a geodesic. Find $\int_{\gamma_2} k_g ds$.

    \noindent \textbf{Solution:}
    By the Global Gauss-Bonnet Theorem:
    \[ \int_{\partial R} k_g ds + \iint_R K dA = 2\pi \chi(R) \]
    Here $\partial R = \gamma_1 \cup \gamma_2$. Since $\gamma_1$ is a geodesic, $\int_{\gamma_1} k_g ds = 0$.
    
    Consider the parametrization $X(\theta, \varphi) = (\cos\theta \cos\varphi, \cos\theta \sin\varphi, \sin\theta)$ with $\theta \in [0, \frac{\pi}{4}]$ and $\varphi \in [0, 2\pi]$.
    \begin{itemize}
        \item This region $R$ is an annulus (a cylinder topologically) bounded by the equator ($\theta=0$) and the parallel $\theta = \pi/4$.
        \item Therefore, $\chi(R) = 0$.
    \end{itemize}
    The Gauss-Bonnet formula becomes:
    \[ \int_{\gamma_2} k_g ds + \iint_R K dA = 0 \implies \int_{\gamma_2} k_g ds = - \iint_R K dA \]
    We compute the area integral. For the sphere, $K=1$.
    Using the first fundamental form for this parametrization:
    \[ X_\theta = (-\sin\theta\cos\varphi, -\sin\theta\sin\varphi, \cos\theta) \implies E = \langle X_\theta, X_\theta \rangle = 1 \]
    \[ X_\varphi = (-\cos\theta\sin\varphi, \cos\theta\cos\varphi, 0) \implies G = \langle X_\varphi, X_\varphi \rangle = \cos^2\theta \]
    \[ F = 0 \]
    The area element is $\sqrt{EG-F^2} d\theta d\varphi = \cos\theta d\theta d\varphi$.
    \[ \iint_R K dA = \int_0^{2\pi} \int_0^{\pi/4} 1 \cdot \cos\theta \, d\theta \, d\varphi = 2\pi [\sin\theta]_0^{\pi/4} = 2\pi \left(\frac{\sqrt{2}}{2}\right) = \sqrt{2}\pi \]
    Therefore:
    \[ \int_{\gamma_2} k_g ds = -\sqrt{2}\pi \]
\end{example}
\begin{example}[Uniqueness of Closed Geodesics on Catenoid]
    For the Catenoid, it has a unique closed geodesic (the central circle at the neck).
    We can prove that no other closed geodesics exist and that this geodesic does not intersect itself or others in a way that forms a bounded region.
    
    Let $\gamma_1$ and $\gamma_2$ be two closed geodesics.
    \begin{enumerate}
        \item \textbf{Case 1: $\gamma_1$ and $\gamma_2$ do not intersect.}
        In this case, the two geodesics bound a region $R$ which is topologically a cylinder ($\chi(R)=0$).
        Applying Gauss-Bonnet:
        \[ \int_{\gamma_1} k_g ds + \int_{\gamma_2} k_g ds + \iint_R K dA = 2\pi \chi(R) = 0 \]
        Since $\gamma_i$ are geodesics, $k_g = 0$. Thus:
        \[ \iint_R K dA = 0 \]
        However, for a Catenoid (a minimal surface that is not a plane), $K < 0$ everywhere. Since the area of $R$ is positive, $\iint_R K dA < 0$.
        This is a contradiction. Thus, two closed geodesics cannot be disjoint.

        \item \textbf{Case 2: Intersect at one point.}
        (This configuration is generally not possible for simple closed geodesics on such surfaces without self-intersection).

        \item \textbf{Case 3: Intersect at more than 1 point.}
        If they intersect at two points, the geodesics bound a region $R$ which is a "bi-gon" (topologically a disk, $\chi(R)=1$).
        
        % [Simple TikZ diagram of two intersecting curves forming a lune/bi-gon]
        \begin{center}
        \begin{tikzpicture}
            \draw[smooth, tension=1] (0,0) to [out=45,in=135] (4,0);
            \draw[smooth, tension=1] (0,0) to [out=-45,in=-135] (4,0);
            \fill (0,0) circle (2pt) node[left] {$p_1$};
            \fill (4,0) circle (2pt) node[right] {$p_2$};
            \node at (2,0) {$R$};
        \end{tikzpicture}
        \end{center}

        Consider the Gauss-Bonnet theorem on this disk $R$. Let the internal angles at the vertices be $\alpha_1, \alpha_2$.
        \[ \iint_R K dA + \sum \theta_{ext} = 2\pi \chi(R) = 2\pi \]
        The external angles are $\pi - \alpha_i$. Thus:
        \[ \iint_R K dA + (\pi - \alpha_1) + (\pi - \alpha_2) = 2\pi \]
        \[ \iint_R K dA = \alpha_1 + \alpha_2 \]
        Since the curves are geodesics coming from the same point, the internal angles $\alpha_i > 0$. Thus RHS $> 0$.
        However, since $K < 0$ everywhere on the Catenoid, $\iint_R K dA < 0$.
        This is a contradiction.
    \end{enumerate}
    Therefore, such closed geodesics cannot exist.
\end{example}

\newpage
\appendix
% New content from 2025-12-12

\appendix
\section{Poincaré-Hopf Theorem}

In this section, we discuss the Poincaré-Hopf Theorem as an application of the Gauss-Bonnet theorem. 

\begin{definition}[Index at a singular point $p$]
    Let $S \subseteq \mathbb{R}^3$ be a regular surface and $X: U \subseteq \mathbb{R}^2 \rightarrow S$ be a coordinate patch. Let $\vec{v}$ be a vector field on $S$. A point $p \in S$ is a \textbf{singular point} if $\vec{v}(p) = 0$. We define the index of $\vec{v}$ at $p$ as:
    \[
    \mathrm{index}_p(\vec{v}) \coloneqq \frac{1}{2\pi} \int_{\gamma} d\theta
    \]
    where $\theta$ is the angle between $\vec{v}(t)$ and $X_u$, and $\gamma$ is any simple closed curve around $p$ that contains only $p$ as the singular point.
\end{definition}

\begin{proposition}
    The above definition is independent of the choice of curve $\gamma$.
\end{proposition}

\begin{proof}
    Note that $\frac{1}{2\pi} \int_{\gamma} d\theta$ is always an integer. We can connect $\gamma_1$ and $\gamma_2$ (two simple closed curves around $p$ containing only $p$ as a singular point) through a continuous map. The value of the integral is constant when we deform a curve $\gamma_1$ to $\gamma_2$, hence it is independent of the choice of $\gamma$.
\end{proof}

\begin{proposition}
    The definition is independent of the choice of coordinate patch.
\end{proposition}

\begin{proof}
    Suppose $\{X_u, X_v\}$ and $\{Y_u, Y_v\}$ are two coordinate frames. Then we have the relationship between angles:
    \[
    \theta = \tilde{\theta} + \alpha
    \]
    where $\alpha = \angle(Y_u, X_u)$. Then we have $d\theta = d\tilde{\theta} + d\alpha$. Note that $\Delta \alpha = \int_{\gamma} d\alpha$. Since the frame is never zero on $\gamma$ and its interior, we can shrink $\gamma$ to a point and have $\Delta \alpha = 0$. By the above definition, we see that $\mathrm{index}_p(\vec{v})$ is well-defined and only depends on $p$ and $\vec{v}$.
\end{proof}

\begin{theorem}[Poincaré-Hopf Theorem]
    Let $S$ be a compact connected surface in $\mathbb{R}^3$. Let $\vec{v}$ be a vector field on $S$ with a finite number of singular points. Then:
    \[
    \sum_{p \in \{q \mid \vec{v}(q)=0\}} \mathrm{index}_p(\vec{v}) = \chi(S)
    \]
    where $\chi(S)$ is the Euler Characteristic.
\end{theorem}

To establish the above astonishing theorem, we need some preparation.

\begin{lemma}
    Let $S$ be a regular surface. Let $\{e_1, e_2\}$ be an orthonormal frame on the surface $S \subseteq \mathbb{R}^3$, with $\{e_1, e_2\}_p \in T_p S$ and $N$ the normal vector to the surface. If $\gamma$ is a simple closed curve on $S$ and $\theta = \angle(\gamma', e_1)$, then:
    \[
    k_g = \dot{\theta} - \langle e_1', e_2 \rangle
    \]
\end{lemma}

\begin{proof}
    Suppose $\gamma' = \cos \theta e_1 + \sin \theta e_2$. Then:
    \[
    \gamma'' = -\sin \theta \dot{\theta} e_1 + \cos \theta \dot{\theta} e_2 + \cos \theta e_1' + \sin \theta e_2'
    \]
    Recall $k_g = \langle \gamma'', N \times \gamma' \rangle$. We have $N \times \gamma' = N \times (\cos \theta e_1 + \sin \theta e_2) = -\sin \theta e_1 + \cos \theta e_2$. Thus:
    \begin{align*}
        k_g &= \langle -\sin \theta \dot{\theta} e_1 + \cos \theta \dot{\theta} e_2 + \cos \theta e_1' + \sin \theta e_2', -\sin \theta e_1 + \cos \theta e_2 \rangle \\
        &= \sin^2 \theta \dot{\theta} - \sin \theta \cos \theta \langle e_1', e_1 \rangle + \cos^2 \theta \dot{\theta} + \cos^2 \theta \langle e_1', e_2 \rangle - \sin^2 \theta \langle e_2', e_1 \rangle + \sin \theta \cos \theta \langle e_2', e_2 \rangle \\
        &= \dot{\theta} - \langle e_1', e_2 \rangle
    \end{align*}
\end{proof}

\begin{remark}
    Recall that when we prove the Gauss-Bonnet theorem, we apply this to $e_1 = \frac{X_u}{\sqrt{E}}$, $e_2 = \frac{X_v}{\sqrt{G}}$ for an orthonormal coordinate system, and obtain $k_g = \dot{\theta} + \frac{1}{2\sqrt{EG}}(G_u \dot{u} - G_v \dot{v})$.
\end{remark}

\begin{proof}[Proof of Poincaré-Hopf theorem]
    First, list all zeros of the vector field as $\{p_1, p_2, \dots, p_n\}$. Let $\{C_i\}_{i=1}^n$ be a collection of simple closed curves that wind around the singular points. Let $D_i$ be the region bounded by $C_i$. Let $X: U \subseteq \mathbb{R}^2 \rightarrow S \setminus \bigcup_{i=1}^n D_i$.
    % [Complex diagram placeholder: Singular points p_i enclosed by curves C_i on a surface]
    Denote $e_1 = \frac{\vec{v}}{\|\vec{v}\|}$ and $e_2$ such that $\{e_1, e_2, N\}$ is an orthonormal system. We claim:
    \[
    \iint_{S \setminus \bigcup_{i=1}^n D_i} K dA = \sum_{i=1}^n \oint_{C_i} \langle e_1, e_1' \rangle
    \]
    By the Lemma, we have $k_g^i = \dot{\theta}_i(s) - \langle e_1, e_1' \rangle$, so $\langle e_1, e_1' \rangle = \dot{\theta}_i(s) - k_g^i$. Also note that as $\mathrm{Area}(D_i) \to 0$ (or $C_i \to p_i$), we have $\oint_{C_i} k_g^i \to 2\pi$. The reason is by the following equations (Local Gauss Bonnet Theorem)
    \[
    \iint_{D_i} K dA + \oint_{C_i} k_g^i = 2\pi
    \]
    Hence we have:
    \[
    \sum_{i=1}^n \oint_{C_i} \langle e_1, e_1' \rangle \longrightarrow \sum_{i=1}^n 2\pi \cdot \mathrm{index}_{p_i}(\vec{v}) - 2\pi n
    \]
    By Gauss-Bonnet we also have:
    \[
    \iint_{S \setminus \bigcup_{i=1}^n D_i} K dA = 2\pi(\chi(S) - n)
    \]
    as $D_i \to 0$. Therefore:
    \[
    2\pi \sum_{i=1}^n \mathrm{index}_{p_i}(\vec{v}) - 2\pi n = 2\pi \chi(S) - 2\pi n
    \]
    Conclude that:
    \[
    \sum_{i=1}^n \mathrm{index}_{p_i}(\vec{v}) = \chi(S)
    \]
    This is the proof of the Poincaré-Hopf theorem by Gauss-Bonnet.
\end{proof}

\begin{remark}
    If you read Liu's lecture notes or \textit{Differential Forms and Its Applications} by Do Carmo, you will find they use $\omega^1, \omega^2, \omega_1^2$ differential forms and the "moving frame" argument to reach this result. You can check this and learn the moving frame method.
\end{remark}

\section{Generalizing the Covariant Derivative to Tensor Fields}

\textbf{From Vectors to Tensors: The Guiding Principles}
...
\begin{enumerate}[label={[\arabic*]}]
    \item Manfredo P. do Carmo, \textit{Differential Geometry of Curves and Surfaces}.
    \item Manfredo P. do Carmo, \textit{Differential Forms and Applications}.
    \item Barrett O'Neill, \textit{Elementary Differential Geometry}.
\end{enumerate}

\subsection*{Online Resources}
\begin{itemize}
    \item Shiping Liu's Differential Geometry Lecture Notes and Videos: \url{http://staff.ustc.edu.cn/~spliu/Teach_DG2017.html}
\end{itemize}
If you are trying to learn more about the moving frame/differential forms and how it was applied in the two dimensional surface, you can read the lecture notes written by Liu or the book differential forms and its applications.
\end{document}
