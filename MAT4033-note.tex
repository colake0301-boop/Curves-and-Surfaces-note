...
\begin{example}[Catenoid-Helicoid Family]
    Let $f(z)=e^{-i\theta}e^z$ and $g(z)=e^{-z}$.
    \[ \phi^1 = \frac{1}{2}f(1-g^2) = \frac{1}{2}e^{-i\theta}e^z(1-e^{-2z}) = \frac{1}{2}e^{-i\theta}(e^z - e^{-z}) = e^{-i\theta} \sinh z \]
    \[ \phi^2 = \frac{i}{2}f(1+g^2) = \frac{i}{2}e^{-i\theta}e^z(1+e^{-2z}) = \frac{i}{2}e^{-i\theta}(e^z + e^{-z}) = i e^{-i\theta} \cosh z \]
    \[ \phi^3 = fg = (e^{-i\theta}e^z)(e^{-z}) = e^{-i\theta} \]
    So, $\phi(z) = (e^{-i\theta} \sinh z, i e^{-i\theta} \cosh z, e^{-i\theta})$.
    
    \begin{itemize}
        \item If $\theta = 0$, $\phi = (\sinh z, i \cosh z, 1)$. Integrating gives the \textbf{Catenoid}.
        \item If $\theta = \frac{\pi}{2}$, $\phi = (-i \sinh z, \cosh z, -i)$. Integrating gives the \textbf{Helicoid}.
    \end{itemize}
\end{example}
...
\begin{remark}
    One can refer this to Do Carmo Chap 3. Exercise.
\end{remark}
...
\newpage
\section{Global Theory of Surfaces}
